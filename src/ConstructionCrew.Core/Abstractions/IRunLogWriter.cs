...
/// <c>&lt;plansFolder&gt;/RUN-LOG.md</c>. The interface lives in Core so
/// HomeOffice can depend on it without referencing Config (where the
/// implementation lives) -- see Architecture §3.7.
///
/// Called from JobRegistry.Transition on a transition into Completed/Failed, and
/// only for a job that actually carried an ActiveWorkorder: an ad-hoc dispatch has
/// no Plans folder to log against and logs nothing.
/// </summary>
