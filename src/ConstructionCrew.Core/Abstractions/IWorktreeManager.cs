...
///
/// Lives in Core so HomeOffice can depend on it without referencing
/// ConstructionCrew.Git, where the real git-shelling implementation lives
/// (Architecture §3.7).
/// </summary>
public interface IWorktreeManager
{
    ...
    /// checks it out into a new worktree at <paramref name="worktreePath"/>.
    ...
    /// in the main worktree. Returns false, never throws, when the merge can't
    /// complete: a conflict is an ordinary outcome the Foreman resolves, not an
    /// exceptional one.
    /// </summary>
